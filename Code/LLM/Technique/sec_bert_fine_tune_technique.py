...
def evaluate_and_save_results(predictions, tokenized_ds, x_test, seed, class_names):
    ...
    with open(f"sec_BERT_Technique_evaluation_metrics_seed_{seed}.txt", "w") as txt_file:
    ...
    results_df.to_csv(f'sec_BERT_Technique_test_predictions_seed_{seed}.csv', index=False)
...
for seed in random_seeds:
  ...
  training_args = TrainingArguments(
      output_dir = f'sec_bert_multilabel_seed_{seed}',
      learning_rate = 1e-5,
...
all_metrics_df.to_csv("sec_BERT_Technique_all_seed_evaluation_metrics.csv", index=False)
