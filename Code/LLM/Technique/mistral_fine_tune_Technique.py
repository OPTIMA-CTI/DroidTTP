...
def evaluate_and_save_results(predictions, tokenized_ds, x_test, seed, class_names):
    ...
    with open(f"mistral_Technique_evaluation_metrics_seed_{seed}.txt", "w") as txt_file:
    ...
    results_df.to_csv(f'mistral_Technique_test_predictions_seed_{seed}.csv', index=False)
...
all_metrics_df.to_csv("mistral_Technique_all_seed_evaluation_metrics.csv", index=False)
