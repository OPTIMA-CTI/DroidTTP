...
def evaluate_and_save_results(predictions, tokenized_ds, x_test, seed, class_names):
    ...
    with open(f"phi_Tactic_evaluation_metrics_seed_{seed}.txt", "w") as txt_file:
    ...
    results_df.to_csv(f'phi_Tactic_test_predictions_seed_{seed}.csv', index=False)
...
random_seeds = [2396]  # Replace with your desired seeds
...
all_metrics_df.to_csv("phi_Tactic_all_seed_evaluation_metrics.csv", index=False)
